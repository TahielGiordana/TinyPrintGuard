...
void RespondToDetection(float person_score, float no_person_score, float string_score) {
  int person_score_int = (person_score) * 100 + 0.5;
  int no_person_score_int = (no_person_score) * 100 + 0.5;
  int string_score_int = (string_score) * 100 + 0.5;
...
#endif
#endif
  MicroPrintf("Output: [%f%%,%f%%,%f%%]", no_person_score,person_score,string_score);
  MicroPrintf("spaghetti score:%d%%, blob score %d%%, string score: %d%% ",
              person_score_int, no_person_score_int, string_score_int);
}
